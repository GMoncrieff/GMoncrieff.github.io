...
aliases:
  - /posts/2021-09-22-sheduled-earth-engine/
---
...
::: {.article__head}
<div class="article__meta"><span>2021 · 11 · 27</span><span>·</span><span>7 min read</span><span>·</span><span style="color:var(--accent);">Remote Sensing</span></div>

# Automate your Google Earth Engine analyses

<p class="hero__tag" style="text-align:left; margin:0; max-width:62ch;">Using Google Cloud Platform to run Earth Engine scripts on a predefined schedule.</p>
:::
...
As a prerequisite you will need a GCP account. If you don't have one you can register [here](https://cloud.google.com/free) and you will start out with $300 of free credits. Most of what we are going to do here using the command-line can also be done via the pointy-clicky method on the GCP web console, but for the sake of simplicity and reproducibility we will use GCP `gcloud` commands for everything. Instructions for installing `gcloud` can be found [here](https://cloud.google.com/sdk/docs/install). Once installed you will need to setup our installation using
...
Now we are ready to upload the function to GCP and deploy using Cloud Functions. Double check that your current working directory contains the following files only: our python function `main.py`, the requirements `requirements.txt`, and the json key `sa-private-key.json`.
...
We can view the function and it's details on the Cloud Functions tab of our GCP web console. When we deployed the function we set a trigger that will cause the function to execute `--trigger-topic=ee_topic`. This created a GCP Pub/Sub topic called `ee_topic`. [Pub/Sub](https://cloud.google.com/pubsub) is GCP's messaging service which integrates different components, some of which publish messages, and others that consume/subscribe to these messages. Our function consumes messages published to `ee_topic`, which will trigger it's execution.
